...
function debounce(callback, ms) {
  ...
  const f = function (...args) {
    ...
    if (debounceTimeout) {
      // We are in a debounce period with a timeout already set.
      // Update the call arguments for the call that will happen
      ...
      // console.log(currentTime, 'debounced call scheduled in ', lastCallTime + ms - currentTime, ' ms');
      // We are in a debounce period and this is the first call.
      lastThis = this;
      lastArgs = args;
      debounceTimeout = setTimeout(function () {
        debounceTimeout = null; // console.log(currentTime, 'debounced call executed, ', Date.now() - lastCallTime, ' ms after last call');
...
/** @lends Sortable.prototype */
{
  ...
  fallback) {
    ...
    _this.cloneId = _nextTick(function () {
      ...
      if (Sortable.eventCanceled) return; // if (!_this.options.removeCloneOnHide) {
  ...
  evt) {
    ...
    : putSortable === this || (this.lastPutMode = activeGroup.checkPull(this, activeSortable, dragEl, evt)) && group.checkPut(this, activeSortable, dragEl, evt))) {
      ...
      if (revert) {
        ...
        dragOverEvent('revert'); // if (!Sortable.eventCanceled) {
        // 	if (nextEl) {
      ...
      if (!elLastChild || _ghostIsLast(evt, vertical, this) && !elLastChild.animated) {
        ...
        if (onMove(rootEl, el, dragEl, dragRect, target, targetRect, evt, !!target) !== false) {
          capture(); // I use nextElementSibling instead if nextSibling because dragNextEl must be an element now.
        ...
        if (onMove(rootEl, el, dragEl, dragRect, target, targetRect, evt, false) !== false) {
          capture(); //el.insertBefore(dragEl, firstChild);
  ...
  evt) {
    let el = this.el,
        options = this.options; // Why wouldn't newIndex be updated? I'll assume it is.
    // // Get the index of the dragged element within its parent
    // newIndex = index(dragEl);
    // newDraggableIndex = index(dragEl, options.draggable);
